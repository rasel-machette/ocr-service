...
public class OcrController {
	...
	@ResponseBody
	public ResponseEntity<List<Invoice>> getInvoices(@RequestParam(name = "owner", required = true) String owner,
	...
	@ResponseBody
	public ResponseEntity<Reponse> processFiles(@RequestBody Archive archive,
			@RequestParam(name = "callbackUrl", required = true) String callbackUrl,
			@RequestParam(name = "owner", required = true) String owner,
			@RequestParam(name = "businessOwner", required = true) String businessOwner)
		...
		/*
		 * for(com.ocr.model.File file: files.getFiles()) { Callback callback = new
		 * Callback(); callback.setUid(id); callback.setCallbackUrl(callbackUrl);
		 * callback.setFileName(file.getName()); callback.setOwner(owner);
		...
		// PredictInvoice predictInvoice = mapper.readValue(jsonNode,
		// PredictInvoice.class);
	...
	@ResponseBody
	public ResponseEntity<Reponse> processInvoices(@RequestBody Invoices invoices)
			throws InvalidKeyException, IOException, ParseException {
		Reponse resp = new Reponse();
		Invoice invoice = new Invoice();
		LOGGER.info("Set invoice: " + invoices.getInvoices().get(0).getFileName() + " :: "
				+ invoices.getInvoices().get(0).getBusinessOwner());
